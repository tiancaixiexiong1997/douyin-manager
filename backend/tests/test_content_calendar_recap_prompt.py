...
async def test_generate_content_calendar_includes_performance_recap_context(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    async def fake_resolve_prompt(**_kwargs: Any) -> tuple[str, dict[str, Any]]:
        return (
            ...
            "字段:priority content_role is_main_validation shoot_format talent_requirement shoot_scene estimated_duration prep_requirement schedule_group\n"
    ...
    assert "priority content_role is_main_validation shoot_format talent_requirement shoot_scene estimated_duration prep_requirement schedule_group" in user_prompt
